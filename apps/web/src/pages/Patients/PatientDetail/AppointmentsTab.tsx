...
export default function AppointmentsTab({ patientName }: { patientName: string }) {
  const { t } = useApp();
  ...
  const columns: Column<Appointment>[] = [
    ...
    { key: 'doctorName', header: t('appointments.form.doctor') },
  ...
  return (
    <div className="space-y-3">
      <p className="text-sm text-muted">{t('patients.detail.descriptions.appointments')}</p>
      {isLoading ? (
